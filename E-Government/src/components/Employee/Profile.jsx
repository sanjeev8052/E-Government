...
const Profile = () => {
    ...
    const { empProfileImage, loading , error , getEmpData} = useSelector(state => state.employee)
    ...
    useEffect(() => {
        getEmpData ? setUser(getEmpData ) : null
    }, [getEmpData  ]);
    ...
    const { values, touched, errors, handleBlur, handleSubmit } = useFormik({
        ...
        onSubmit: () => {
            dispatch(updateEmpProfile(user, user._id))
